...
import { Users, Clock, TrendingUp, Activity, ChevronDown } from 'lucide-react';
...
const AnalyticsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchActivity = async () => {
      ...
      try {
        ...
        if (!token) return;
        const resp = await fetch(`http://localhost:3000/api/auth/analytics/activity?range=${range}`, {
  ...
  return (
    <div className="min-h-screen text-white relative font-sans selection:bg-lime-500/30">
      {/* Live 3D Data Environment Background */}
      <div className="fixed inset-0 pointer-events-none -z-10 overflow-hidden bg-[#050505]">
        {/* Deep Atmospheric Glows */}
        <div className="absolute -top-[20%] -left-[10%] w-[800px] h-[800px] bg-lime-900/10 rounded-full blur-[150px] mix-blend-screen animate-pulse" style={{ animationDuration: '8s' }} />
        <div className="absolute top-[20%] right-[-20%] w-[600px] h-[600px] bg-emerald-900/10 rounded-full blur-[120px] mix-blend-screen animate-pulse" style={{ animationDuration: '10s' }} />

        {/* 3D Perspective Grid Floor */}
        <div className="absolute inset-0 bottom-[-40%] opacity-20"
          style={{
            transform: 'perspective(1000px) rotateX(60deg) scale(2)',
            transformOrigin: 'bottom center',
            maskImage: 'linear-gradient(to_bottom, transparent 0%, black 50%, transparent 100%)'
          }}>
          {/* Moving Grid Lines */}
          <div className="absolute inset-0 bg-[linear-gradient(to_right,rgba(132,204,22,0.1)_1px,transparent_1px),linear-gradient(to_bottom,rgba(132,204,22,0.1)_1px,transparent_1px)] bg-[size:60px_60px] animate-[grid-fly_20s_linear_infinite]" />
        </div>

        {/* Live Growing Graph Animation (Background) */}
        <svg className="absolute inset-0 w-full h-full opacity-30" viewBox="0 0 1000 1000" preserveAspectRatio="none">
          ...
          {/* Main Increasing Trend Line */}
          <path
            d="M-100,900 C150,850 300,950 500,700 S800,500 1200,200"
            fill="none"
            stroke="url(#lineGrad)"
            strokeWidth="3"
            filter="url(#glow-line)"
            className="drop-shadow-[0_0_15px_rgba(132,204,22,0.4)]"
          >
            <animate attributeName="d"
              values="M-100,900 C150,850 300,950 500,700 S800,500 1200,200;
                      M-100,910 C150,860 300,960 500,720 S800,520 1200,220;
                      M-100,900 C150,850 300,950 500,700 S800,500 1200,200"
              dur="8s"
          ...
          {/* Secondary Echo Line */}
          <path
            d="M-100,920 C150,870 300,970 500,720 S800,520 1200,220"
            fill="none"
        ...
        {/* Style block for background-specific animations */}
        <style>{`
          @keyframes grid-fly {
          ...
          <div className="relative rounded-[2rem] bg-gray-900/40 border border-white/5 backdrop-blur-xl p-8 shadow-2xl h-[500px] hover:border-white/10 transition-colors duration-700">
            ...
            {loading ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={buckets} margin={{ top: 20, right: 0, left: -20, bottom: 0 }}>
                  <defs>
